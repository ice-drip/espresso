...
describe('Invalid key handling', () => {
  ...
  it('AES with wrong key length (10 bytes) does not throw', () => {
    ...
    const ciphertext = encrypt('aes', 'Hello, World!', key10, { mode: 'cbc', iv, outputEncoding: 'hex' });
  ...
  it('decrypts with wrong key throws Invalid PKCS7 padding', () => {
    ...
    const ciphertext = encrypt('aes', plaintext, key16, { mode: 'cbc', iv, outputEncoding: 'hex' }) as string;
    expect(() => decrypt('aes', ciphertext, wrongKey, { mode: 'cbc', iv, outputEncoding: 'utf8' })).toThrow('Invalid PKCS7 padding');
  });
});
