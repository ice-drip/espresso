...
const SEED_SS: number[][] = [
  ...
  [
    ...
    0xdc1dcdd1, 0x50114151, 0x90108090, 0xd1cccd0, 0x981a8a92, 0xa02383a3,
    ...
    0xf033c3f3, 0x48094941, 0x78384870, 0xcc0ccc0, 0x14150511, 0xf83bcbf3,
    ...
    0xe82acae2, 0x08090901, 0x74364672, 0x18190911, 0xfc3ece32, 0x40004040,
  ...
  [
    ...
    0x90901080, 0x62682a4a, 0x22282a0a, 0x1a8a9298
  ],
  [
...
function seedBlock(data: Uint8Array, key: Uint8Array, doEncrypt: boolean): Uint8Array {
  ...
  for (let i = 0; i < 16; i++) {
    K[i] = [
      SEED_G((A + C - SEED_KC[i]) >>> 0),
      SEED_G((B - D + SEED_KC[i]) >>> 0)
    ];
  ...
  const roundKeys = doEncrypt ? K : [...K].reverse();

  const result = new Uint8Array(data.length);
  for (let offset = 0; offset < data.length; offset += 16) {
    const L = [
      ((data[offset] << 24) | (data[offset + 1] << 16) | (data[offset + 2] << 8) | data[offset + 3]) >>> 0,
      ((data[offset + 4] << 24) | (data[offset + 5] << 16) | (data[offset + 6] << 8) | data[offset + 7]) >>> 0
    ];
    const R = [
      ((data[offset + 8] << 24) | (data[offset + 9] << 16) | (data[offset + 10] << 8) | data[offset + 11]) >>> 0,
      ((data[offset + 12] << 24) | (data[offset + 13] << 16) | (data[offset + 14] << 8) | data[offset + 15]) >>> 0
    ];
    ...
    result[offset] = (LR[0][0] >>> 24) & 0xff;
    result[offset + 1] = (LR[0][0] >>> 16) & 0xff;
    result[offset + 2] = (LR[0][0] >>> 8) & 0xff;
    result[offset + 3] = LR[0][0] & 0xff;
    result[offset + 4] = (LR[0][1] >>> 24) & 0xff;
    result[offset + 5] = (LR[0][1] >>> 16) & 0xff;
    result[offset + 6] = (LR[0][1] >>> 8) & 0xff;
    result[offset + 7] = LR[0][1] & 0xff;
    result[offset + 8] = (LR[1][0] >>> 24) & 0xff;
    result[offset + 9] = (LR[1][0] >>> 16) & 0xff;
    result[offset + 10] = (LR[1][0] >>> 8) & 0xff;
    result[offset + 11] = LR[1][0] & 0xff;
    result[offset + 12] = (LR[1][1] >>> 24) & 0xff;
    result[offset + 13] = (LR[1][1] >>> 16) & 0xff;
    result[offset + 14] = (LR[1][1] >>> 8) & 0xff;
    result[offset + 15] = LR[1][1] & 0xff;
  }
  return result;
}

export function seedEncrypt(data: Uint8Array, key: Uint8Array, opts: CipherOptions): Uint8Array {
  const padding = opts.padding ?? 'pkcs7';
  const padded = padding === 'pkcs7' ? pkcs7Pad(data, 16) : data;
  return seedBlock(padded, key, true);
}

export function seedDecrypt(data: Uint8Array, key: Uint8Array, opts: CipherOptions): Uint8Array {
  const result = seedBlock(data, key, false);
  const padding = opts.padding ?? 'pkcs7';
  if (padding === 'pkcs7') return pkcs7Unpad(result, 16);
  return result;
}
