...
function desDoCryptBlock(
  ...
  for (let round = 0; round < 16; round++) {
    ...
    let f = 0;
    for (let i = 0; i < 8; i++) {
      f |= SBOX_P[i][((r ^ subKey[i]) & SBOX_MASK[i]) >>> 0];
    }
    const newL = r;
    const newR = l ^ f;
    l = newL;
...
function desEncryptBlock(block: Uint8Array, subKeys: number[][]): void {
  ...
  block[0] = (result.lBlock >>> 24) & 0xff;
  block[1] = (result.lBlock >>> 16) & 0xff;
  block[2] = (result.lBlock >>> 8) & 0xff;
  block[3] = result.lBlock & 0xff;
  block[4] = (result.rBlock >>> 24) & 0xff;
  block[5] = (result.rBlock >>> 16) & 0xff;
  block[6] = (result.rBlock >>> 8) & 0xff;
  block[7] = result.rBlock & 0xff;
}
