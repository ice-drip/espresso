...
function sha512Block(h: bigint[], block: Uint8Array): void {
  ...
  for (let i = 0; i < 80; i++) {
    ...
    const notE = e ^ 0xffffffffffffffffn;
    const ch = (e & f) ^ (notE & g);
    const temp1 = (hh + S1 + ch + K[i] + w[i]) & 0xffffffffffffffffn;
...
function sha512Hash(data: Uint8Array, outputBytes: number): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLen; offset += 128) {
    sha512Block(h, padded.subarray(offset, offset + 128));
  }

  const result = new Uint8Array(outputBytes);
  const fullWords = Math.floor(outputBytes / 8);
