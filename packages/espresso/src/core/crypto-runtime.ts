...
export async function webCryptoEncrypt(
  ...
  const cryptoKey = await subtle.importKey("raw", key as BufferSource, algo, false, ["encrypt"]);
...
export async function webCryptoDecrypt(
  ...
  const cryptoKey = await subtle.importKey("raw", key as BufferSource, algo, false, ["decrypt"]);

  const decrypted = await subtle.decrypt(algo, cryptoKey, data as BufferSource);
  return new Uint8Array(decrypted);
}

export async function webCryptoHash(algorithm: string, data: Uint8Array): Promise<Uint8Array> {
  const subtle = globalThis.crypto.subtle;
